...
export const divide = (divisor: number): Endomorphism<number> => (dividend: number) => dividend / divisor;

export const multiply = (by: number): Endomorphism<number> => (value: number) => value * by;

export const isValid = P.not(Number.isNaN);

export const fromStringWithRadix = (radix: number) => (str: string): Option<number> =>
