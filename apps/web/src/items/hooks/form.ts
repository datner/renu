...
const revalidate = Effect.sync(() =>
  navigator.sendBeacon("/api/revalidate-current")
);
  ...
  const onSubmit = useCallback(
    (data: ItemSchema) => {
      ...
      return pipe(
        ...
        Effect.tap((item) =>
          Effect.sync(() =>
            setQueryData(getItem, { identifier: item.identifier }, item)
          )
        ),
...
const useUpdate = (identifier: string) => {
  ...
  const onSubmit = (data: ItemSchema) =>
    pipe(
      ...
      Effect.flatMap(() =>
        Effect.promise(() => invoke(updateItem, { id: item.id, ...data }))
      ),
      ...
      Effect.tap((item) =>
        Effect.ifEffect(
          ...
          Effect.unit(),
          Effect.sync(() =>
            router.push(Routes.AdminItemsItem({ identifier: item.identifier }))
          ),
        )
      ),
      Renu.runPromise$,
