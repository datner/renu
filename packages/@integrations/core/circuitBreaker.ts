...
) => <R, A>(self: Effect.Effect<R, E, A>) => Effect.Effect<R, E | CircuitBreakerError, A>
...
export const makeBreaker = (config?: BreakerConfig | undefined) =>
  Effect.gen(function* ($) {
    ...
    return <E>(isRetryable: Predicate.Predicate<E>) =>
      <R, A>(self: Effect.Effect<R, E, A>) =>
        pipe(
          Ref.updateAndGet(
            state,
            onOpen((s) => (Duration.lessThan(s.endTime, now()) ? closed(maxFailure) : s))
          ),
          Effect.flatMap(
            foldBreaker(
              () =>
                Effect.tapError(self, (e) =>
                  Ref.update(
                    state,
                    onClosed((s) =>
                      isRetryable(e) && s.failCount < maxFailure
          ...
          Effect.retry(
            Schedule.checkEffect(retry as Schedule.Schedule<R, CircuitBreakerError | E, A>, () =>
