...
export const taggedError = <T extends string>(_tag: T) => (error: unknown): TaggedError<T> => ({
  _tag,
  error,
});
...
export const isTaggedErrorFromUnknown =
  <Tag extends string>(tag: Tag): Predicate.Refinement<unknown, TaggedError<Tag>> => (err): err is TaggedError<Tag> =>
